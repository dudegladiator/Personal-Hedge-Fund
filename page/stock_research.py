...
# --- Helper function for consistent recommendation display ---
def display_recommendation(title, recommendation, confidence, rationale=None, color_map=None):
    ...
    try:
        ...
        # Handle None recommendation gracefully
        recommendation_str = recommendation if recommendation else "N/A"
        color = color_map.get(recommendation, "grey") # Default color
        ...
        # else: No confidence provided - do nothing

        # Handle rationale safely
        if rationale:
            st.markdown("**Rationale / Overview:**")
            st.markdown(str(rationale)) # Convert rationale to string just in case
    except Exception as e:
        st.warning(f"Error displaying recommendation element: {e}")


# --- Initialize Session State for Stock Research ---
...
def render_stock_research():
    ...
    try: # Wrap the whole page render in a try-except as a last resort
        initialize_stock_research_session() # Ensure state is initialized
        ...
        with st.expander("Analysis Parameters", expanded=False):
             # Inputs within expander are generally safe unless callbacks cause issues
        ...
        if st.session_state.stock_research_query and not st.session_state.selected_stock_for_analysis:
            ...
            try:
                ...
                search_results = None # Ensure it's None on error
            ...
            if search_results is None:
            ...
            elif not search_results: # Handles empty list case
                st.info(f"No NSE stock results found matching \"{query}\".")
            else: # Results exist and is a list
                ...
                for i, stock in enumerate(search_results):
                    # Add basic check if stock is a dictionary before accessing keys
                    if isinstance(stock, dict):
                         with results_cols[col_idx % 2]:
                             with st.container(border=True, height=180):
                                 try: # Wrap individual item display
        ...
        # --- 4. Run and Display Analysis Results (If stock selected) ---
        selected_stock_info = st.session_state.selected_stock_for_analysis
        if selected_stock_info:
            # Validate selected stock info again
            if not isinstance(selected_stock_info, dict) or 'Ticker' not in selected_stock_info:
                 ...
                 st.session_state.selected_stock_for_analysis = None # Reset invalid state
                 st.rerun()
                 return

            ticker_to_analyze = selected_stock_info['Ticker']
            stock_name_to_analyze = selected_stock_info.get('Name', ticker_to_analyze) # Use ticker if name missing
            ...
            if should_run_analysis:
                ...
                with st.spinner(f"Performing AI analysis for {ticker_to_analyze}..."):
                     ...
                     try:
                        ...
                        # Minimal check on result structure
                        ...
                        try:
                        ...
                        except Exception as e_bt: # Catch specific backtesting errors
                             st.error(f"Backtesting analysis error: {e_bt}")
                             analysis_results['backtesting'] = {"error": True, "details": str(e_bt)}
                             if "talib" in str(e_bt).lower(): st.warning("TA-Lib might be missing/misconfigured.")


                     except Exception as agent_exec_err: # Catch errors during the sequence of agent calls
                         ...
                         if run_success:
                              ...
                              time.sleep(0.5)
                         # Always reset the button click flag
                         st.session_state.analyze_button_clicked = False
                         # Rerun only if successful to display results
                         if run_success:
                              st.rerun()
            ...
            if results: # Check if results dictionary exists
                # Check for top-level error marker from failed sequence
                ...
                if sentimental_results is None: st.info("Sentimental analysis data unavailable.")
                elif not isinstance(sentimental_results, dict): st.warning("Invalid format for sentimental results.")
                elif sentimental_results.get("error"): st.error(f"Sentimental Analysis Error: {sentimental_results.get('message', 'Unknown')}")
                else:
                    try: # Wrap display block
                        ...
                        with senti_cols[0]:
                            ...
                            if isinstance(news_analysis, dict) and not news_analysis.get("error"):
                                display_recommendation(title="", recommendation=news_analysis.get("recommendation_sign"), confidence=news_analysis.get("recommendation_confidence_score"), rationale=news_analysis.get("analysis_overview"))
                            elif isinstance(news_analysis, dict) and news_analysis.get("error"): st.warning(f"News analysis issue: {news_analysis.get('message', 'N/A')}")
                            else: st.caption("No news analysis data.")
                        with senti_cols[1]:
                            st.markdown("**Announcements Sentiment**")
                            if isinstance(announce_analysis, dict) and not announce_analysis.get("error"):
                                display_recommendation(title="", recommendation=announce_analysis.get("recommendation_sign"), confidence=announce_analysis.get("recommendation_confidence_score"), rationale=announce_analysis.get("analysis_overview"))
                            elif isinstance(announce_analysis, dict) and announce_analysis.get("error"): st.warning(f"Announcement issue: {announce_analysis.get('message', 'N/A')}")
                            else: st.caption("No announcement data.")
                        ...
                        if raw_senti_news or raw_senti_announce:
                             with st.expander("Raw Sentiment Data"):
                                  if raw_senti_news: st.markdown("**News Articles:**"); st.json(raw_senti_news)
                                  if raw_senti_announce: st.markdown("**Announcements:**"); st.json(raw_senti_announce)
                    except Exception as display_err: st.warning(f"Error displaying sentimental results: {display_err}")

                # --- Display Fundamental Analysis ---
                st.divider()
                st.subheader("📈 Fundamental Analysis")
                fundamental_results = results.get('fundamental')
                if fundamental_results is None: st.info("Fundamental analysis data unavailable.")
                elif not isinstance(fundamental_results, dict): st.warning("Invalid format for fundamental results.")
                elif fundamental_results.get("error"): st.error(f"Fundamental Analysis Error: {fundamental_results.get('error', 'Unknown')}") # Example uses 'error' key
                else:
                    try: # Wrap display block
                         exec_summary = fundamental_results.get('executive_summary', {})
                         if isinstance(exec_summary, dict):
                              display_recommendation(title="Overall Recommendation", recommendation=exec_summary.get("overall_signal_recommendation"), confidence=exec_summary.get("overall_signal_recommendation_confidence_pct"), rationale=exec_summary.get("overall_health_assessment_summary"))
                              st.markdown("**Key Rationale Points:**")
                              rationale_points = exec_summary.get('key_rationale', [])
                              if rationale_points:
                                  for point in rationale_points: st.markdown(f"- {point}")
                              else: st.caption("No specific rationale points.")
                         else: st.warning("Executive summary missing or invalid.")

                         detailed_analysis = fundamental_results.get('detailed_analysis', {})
                         if isinstance(detailed_analysis, dict) and detailed_analysis:
                              st.markdown("---"); st.markdown("**Detailed Category Analysis:**")
                              ...
                              for category, details in detailed_analysis.items():
                                  if isinstance(details, dict):
                                       ...
                                       with col_to_use:
                                            ...
                                            display_recommendation(title="Signal", recommendation=details.get('signal'), confidence=details.get('confidence_pct'))
                                            st.caption(str(details.get('summary', '')))
                                  i += 1

                         raw_data = fundamental_results.get('raw_data', {})
                         if isinstance(raw_data, dict) and raw_data:
                              with st.expander("Raw Fundamental Metrics"):
                                   for ratio_type, data in raw_data.items():
                                       if isinstance(data, dict):
                                           st.markdown(f"**{str(ratio_type).replace('_', ' ').title()}**")
                                           params = data.get('Parameters', {})
                                           if isinstance(params, dict) and params:
                                               param_df = pd.DataFrame(params.items(), columns=['Metric', 'Value'])
                                               try:
                                                   param_df['Value'] = pd.to_numeric(param_df['Value'], errors='ignore')
                                                   st.dataframe(param_df.style.format({"Value": "{:.2f}"}), hide_index=True, use_container_width=True)
                                               except: st.dataframe(param_df, hide_index=True, use_container_width=True)
                                           else: st.caption("No parameters found.")
                                           st.caption(f"Signal: {data.get('Signal', 'N/A')}, Confidence: {data.get('Confidence Level (%)', 'N/A')}%")
                                           st.markdown("---")
                    except Exception as display_err: st.warning(f"Error displaying fundamental results: {display_err}")
                ...
                if backtesting_results is None: st.info("Backtesting analysis data unavailable.")
                elif not isinstance(backtesting_results, dict): st.warning("Invalid format for backtesting results.")
                elif backtesting_results.get("error"): st.error(f"Backtesting Analysis Error: {backtesting_results.get('details', 'Unknown')}")
                else:
                     try: # Wrap display block
                         bt_exec_summary = backtesting_results.get('executive_summary', {})
                         if isinstance(bt_exec_summary, dict):
                              display_recommendation(title="Overall Assessment", recommendation=bt_exec_summary.get("recommendation_sign"), confidence=bt_exec_summary.get("recommendation_confidence"), rationale=bt_exec_summary.get("overall_summary"))
                              st.markdown("**Key Observations:**")
                              observations = bt_exec_summary.get('key_observations', [])
                              if observations and isinstance(observations, list):
                                  for obs in observations: st.markdown(f"- {obs}")
                              else: st.caption("No specific observations.")
                         else: st.warning("Backtesting summary missing or invalid.")
                         ...
                         if isinstance(strategy_analysis_list, list) and strategy_analysis_list:
                              st.markdown("---"); st.markdown("**Strategy Performance:**")
                              for i, strategy_data in enumerate(strategy_analysis_list):
                                   if isinstance(strategy_data, dict):
                                       ...
                                       with st.expander(f"Strategy: {strategy_name}", expanded=(i==0)):
                                           display_recommendation(title="Signal", recommendation=strategy_data.get('signal'), confidence=strategy_data.get('confidence'))
                                           st.markdown("**Summary:**"); st.caption(str(strategy_data.get('summary', '')))

                                           raw_metrics_data = None
                                           try: # Find corresponding raw metrics safely
                                                if isinstance(raw_bt_data_list, list):
                                                     if i < len(raw_bt_data_list) and isinstance(raw_bt_data_list[i], dict) and raw_bt_data_list[i].get('strategy_name') == strategy_name: raw_metrics_data = raw_bt_data_list[i].get('backtest_results')
                                                          ...
                                                          if found_raw: raw_metrics_data = found_raw
                                           except Exception as find_metric_err: st.caption(f"Note: issue finding raw metrics ({find_metric_err})")
                                           ...
                                           if isinstance(raw_metrics_data, dict):
                                                ...
                                                def format_metric(value, suffix='%', decimals=2):
                                                    try: # Safely format
                                                         if isinstance(value, (int, float)): return f"{value * 100 if suffix == '%' else value:.{decimals}f}{suffix if suffix else ''}"
                                           ...
                                           else: st.caption("Detailed metrics not found.")
                                   else: st.warning("Invalid strategy data format.")
                         # else: No strategy analysis or invalid format

                         if isinstance(raw_bt_data_list, list) and raw_bt_data_list:
                              with st.expander("Raw Backtesting Data"): st.json(raw_bt_data_list)
                     except Exception as display_err: st.warning(f"Error displaying backtesting results: {display_err}")


                # --- Add Trade Popup Section ---
                st.divider()
                st.subheader(f"🛒 Paper Trade {ticker_to_analyze}")
                try: # Wrap the API call and popup rendering
                     ...
                     if live_price_data and isinstance(live_price_data, dict) and live_price_data.get('ltp') is not None:
                          popup_stock_data = {
                              ...
                              "Change": live_price_data.get('percentChange', 0.0) # Adjust key if needed
                          }
        ...
        # --- End of selected_stock_info block ---
        ...
        # print(f"Stock Research Page Error: {traceback.format_exc()}")